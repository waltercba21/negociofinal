...
confirmarPedidoYDescontarStock: (id_pedido, callback) => {
  pool.getConnection((err, conn) => {
    ...
    conn.beginTransaction((errTx) => {
      ...
      // 1) Confirmar SOLO si estaba pendiente (evita doble click/doble descuento)
      const qUpdateEstado = `
        UPDATE carritos
        SET estado='confirmado', actualizado_en=NOW()
        WHERE id=? AND estado='pendiente'
      `;
      conn.query(qUpdateEstado, [id_pedido], (errUp, rUp) => {
        if (errUp) return rollback(errUp);

        if (!rUp.affectedRows) {
          const e = new Error('El pedido no está en estado pendiente (ya confirmado o no válido).');
          e.code = 'NO_PENDIENTE';
          return rollback(e);
        }

        // 2) Traer items y bloquear stock de productos
        const qItems = `
          SELECT
            pc.producto_id,
            pc.cantidad,
          ...
          FOR UPDATE
        `;
        conn.query(qItems, [id_pedido], (errIt, items) => {
          if (errIt) return rollback(errIt);
          if (!items || items.length === 0) {
            const e = new Error('El pedido no tiene productos.');
            e.code = 'SIN_ITEMS';
            return rollback(e);
          }
          ...
          for (const it of items) {
            ...
            if (cant <= 0) {
              const e = new Error('Cantidad inválida en el pedido.');
              e.code = 'CANT_INVALIDA';
              return rollback(e);
            }

            if (stock < cant) {
              const e = new Error(`Stock insuficiente para producto ${it.producto_id}.`);
              e.code = 'STOCK_INSUFICIENTE';
              e.detalle = { producto_id: it.producto_id, stock_actual: stock, solicitado: cant };
              return rollback(e);
            }
          }

          // 4) Descontar stock (ya validado y bloqueado)
          let i = 0;
          const descontarSiguiente = () => {
            if (i >= items.length) {
            ...
            const it = items[i++];
            const qDesc = `UPDATE productos SET stock_actual = stock_actual - ? WHERE id = ?`;
            conn.query(qDesc, [it.cantidad, it.producto_id], (errDesc) => {
              if (errDesc) return rollback(errDesc);
              descontarSiguiente();
            });
          };
