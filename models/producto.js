...
obtenerProductosAsignadosProveedor: function(conexion, proveedor) {
    const query = `
        SELECT pp.codigo AS codigo_proveedor, p.nombre
        FROM productos p
        INNER JOIN producto_proveedor pp ON p.id = pp.producto_id
        WHERE (p.id, pp.proveedor_id) IN (
            SELECT p2.id, pp2.proveedor_id
    ...
    return queryPromise(query, [proveedor])
        .then(result => {
            console.log('Resultados de obtenerProductosAsignadosProveedor:', result);
            return result;
        });
},
