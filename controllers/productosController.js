...
// ✅ CONTROLADOR CORREGIDO: productosController.js > actualizarPreciosExcel
            ...
            if (typeof precioRaw === 'string') {
              ...
              promises.push(
                producto.obtenerProductoPorCodigo(codigo).then(async (productoBD) => {
                  ...
                  // Armar objeto para actualización
                  ...
                  // Calcular nuevo precio de venta con utilidad del producto
                  const descuentoProveedor = await producto.obtenerDescuentoProveedor(proveedor_id);
                  const costoNeto = precio - (precio * (descuentoProveedor / 100));
                  const costoIVA = costoNeto * 1.21;
                  const precioVenta = costoIVA + (costoIVA * (productoBD.utilidad / 100));

                  const resultado = await producto.actualizarPreciosPDF(conexion, productoActualizar, precioVenta);

                  if (resultado?.actualizado) {
                    productosActualizados.push({ codigo, precio_nuevo: precioVenta });
                  } else {
                    productosActualizados.push({ codigo, sinCambio: true });
                  }
                })
              );
            } else {
