...
generarListadoProveedorAsignado: async function (req, res) {
    ...
    try {
        ...
        const obtenerProductos = producto.obtenerProductosAsignadosProveedor(conexion, proveedorId);
    ...
    buffer.on('finish', function() {
        const pdfData = buffer.getContents();
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', 'attachment; filename=productos_asignados.pdf');
        res.send(pdfData);
    });
},
