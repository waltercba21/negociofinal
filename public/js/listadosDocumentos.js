document.addEventListener('DOMContentLoaded', () => {
  ...
  const filtroCondicionEl = document.getElementById('filtroCondicion');
  ...
  const tipoNCLabel = (t) => {
    if (t === 'descuento') return 'Por descuento';
    if (t === 'devolucion_mercaderia') return 'Por devolución de mercadería';
    if (t === 'diferencia_precio') return 'Por diferencia de precio';
    return t || '';
  };
  ...
  const getDetalleUrl = (tipo, id) => {
    if (tipo === 'nota_credito') return `/administracion/api/notas-credito/${id}`;
    return `/administracion/api/${tipo}/${id}`; // factura / presupuesto
  };

  const getCrudUrl = (tipo, id) => {
    if (tipo === 'nota_credito') return `/administracion/api/notas-credito/${id}`;
    return `/administracion/api/${tipo}/${id}`; // PUT/DELETE factura/presupuesto
  };

  // UX: si eligen Nota de Crédito, la condición no aplica
  if (filtroTipoEl && filtroCondicionEl) {
    filtroTipoEl.addEventListener('change', () => {
      const isNC = filtroTipoEl.value === 'nota_credito';
      filtroCondicionEl.disabled = isNC;
      if (isNC) filtroCondicionEl.value = '';
    });
  }
    ...
    try {
      ...
      if (tipo === 'nota_credito') {
        const queryNC = new URLSearchParams({
          ...
          desde: fechaDesde,
          hasta: fechaHasta,
          tipo: '',         // filtro por tipo de NC (descuento / devolucion / diferencia) no está en tu UI todavía
          iva: '',          // filtro iva no está en tu UI todavía
          numeroNC: numero, // usamos el campo "Número" para número de NC
          numeroFactura: '' // filtro por factura asociada no está en tu UI todavía
        });

        const res = await fetch(`/administracion/api/notas-credito?${queryNC.toString()}`);
      ...
      // =========================
      // FACTURAS / PRESUPUESTOS (tu flujo actual)
      // =========================
  ...
  function renderizarPagina(pagina) {
    ...
    datosPagina.forEach(doc => {
      ...
      const extraNC = doc.tipo === 'nota_credito'
        ? `
          <p><strong>Factura:</strong> ${doc.numero_factura || '-'}</p>
          <p><strong>Tipo:</strong> ${tipoNCLabel(doc.tipo_nc)}</p>
          <p><strong>IVA:</strong> ${doc.iva || '-'}%</p>
          <p><strong>Importe:</strong> $${moneyARS(doc.importe)}</p>
        `
        : '';
  ...
  // Mantengo tu listener de "futuro" (no molesta)
  ...
  // ✅ ESTE es el que trae detalle (lo amplié para nota_credito)
  document.addEventListener('click', async (e) => {
    if (e.target.classList.contains('ver-mas-documento')) {
      ...
      try {
        const res = await fetch(getDetalleUrl(tipo, id));
  ...
  function renderDetalleDocumento(data, tipo) {
    ...
    // Si es nota de crédito, por ahora oculto imprimir (no definimos PDF para NC)
    if (tipo === 'nota_credito') {
      if (btnPrint) btnPrint.classList.add('d-none');
    } else {
      if (btnPrint) {
        btnPrint.classList.remove('d-none');
        btnPrint.href = `/administracion/pdf/${tipo}/${data.id}`;
      }
    }
    ...
    if (tipo === 'nota_credito') {
        ...
        <form id="formDetalleDocumento">
          ...
          <div class="row">
            ...
            <div class="col-md-6 mb-2">
              ...
              <input type="text" class="form-control" name="numero_factura" value="${data.numero_factura || ''}" disabled>
            </div>
          </div>
          ...
          <div class="row">
            ...
            <div class="col-md-4 mb-2">
              ...
              <input type="number" step="0.01" name="importe_total" class="form-control" value="${data.importe_total || 0}" disabled>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col-md-6 mb-2">
              ...
              <select name="tipo" class="form-select" disabled>
                <option value="descuento" ${data.tipo === 'descuento' ? 'selected' : ''}>Por descuento</option>
                <option value="devolucion_mercaderia" ${data.tipo === 'devolucion_mercaderia' ? 'selected' : ''}>Por devolución de mercadería</option>
                <option value="diferencia_precio" ${data.tipo === 'diferencia_precio' ? 'selected' : ''}>Por diferencia de precio</option>
              </select>
            </div>
            <div class="col-md-6 mb-2">
              <label>IVA</label>
      ...
      // dataset para editar/eliminar/guardar
    ...
    // =========================
    // DETALLE FACTURA / PRESUPUESTO (tu flujo actual)
    // =========================
  ...
  document.getElementById('btnGuardarCambiosDocumento').addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(getCrudUrl(tipo, id), {
        method: 'PUT',
  ...
  // ✅ Tus botones PDF resumen quedan igual
  ...
  document.getElementById('btnEliminarDocumento').addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(getCrudUrl(tipo, id), { method: 'DELETE' });
      if (!res.ok) throw new Error();
  ...
  // Tu vencimientos queda igual (facturas/presupuestos)
