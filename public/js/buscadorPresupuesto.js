...
document.getElementById('entradaBusqueda').addEventListener('input', async (e) => {
  ...
  productos.forEach((producto) => {
    ...
    resultado.addEventListener('click', () => {
      ...
      filaFactura.insertCell(2).textContent = formatCurrency(producto.precio_venta);
      ...
      filaFactura.insertCell(4).textContent = formatCurrency(producto.precio_venta);

      calcularTotal();

      inputCantidad.addEventListener('change', () => {
        const cantidad = parseInt(inputCantidad.value.trim());
        filaFactura.cells[4].textContent = formatCurrency(cantidad * parseFloat(producto.precio_venta));
        calcularTotal();
      });
