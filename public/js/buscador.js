...
document.getElementById('entradaBusqueda').addEventListener('input', (e) => {
    ...
    timer = setTimeout(async () => {
        ...
        // Limpiar solo si no hay búsqueda
        if (!busqueda.trim()) {
            mostrarProductos(productosOriginales.slice(0, 12)); // Mostrar los primeros 12 productos si no hay búsqueda
        } else {
            const url = '/productos/api/buscar?q=' + encodeURIComponent(busqueda);
            try {
                const respuesta = await fetch(url);
                const productos = await respuesta.json();
                mostrarProductos(productos); // Mostrar los productos buscados
            } catch (error) {
...
// Función para mostrar los productos sin borrar los anteriores
