...
(() => {
  ...
  function setupCategoriasCarousel() {
    ...
    const setIndicador = (idx) => {
      ...
      // Limpia “activo” si existe
      indicadores.forEach((n) => n.classList && n.classList.remove("activo"));

      // Marca el actual si coincide en cantidad
    ...
    // Click navegación (delegado, evita duplicados)
    scroller.addEventListener("click", (e) => {
      // Si fue drag, ignorar click
      if (dragGuard.isDragClick()) return;

      const pelicula = e.target.closest(".pelicula");
      if (!pelicula) return;

      // Prioridad: data-categoria en .imagen-carrusel
    ...
    scroller.addEventListener("pointermove", (e) => {
      ...
      // Evita selección accidental en drag
    ...
    if (canMouseMoveScroll) {
      scroller.addEventListener("mousemove", (e) => {
        ...
        rafMove = requestAnimationFrame(() => {
          ...
          // Movimiento sutil: interpola hacia el target
      ...
      scroller.addEventListener("mouseleave", () => {
        // al salir, vuelve a snap y reanuda
        scrollToIndex(getActiveIndex(), "smooth");
        startAutoplay();
      });
    }
