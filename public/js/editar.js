...
function llenarModelos(marca, modeloSeleccionado) {
    ...
    for (var i = 0; i < modelos.length; i++) {
        ...
        if (modelos[i] == modeloSeleccionado) {
...
document.getElementById('marca-input').addEventListener('change', function() {
    llenarModelos(this.value);
});

// Llena los modelos cuando la página se carga por primera vez
llenarModelos(document.getElementById('marca-input').value, "<%= producto.modelo %>");