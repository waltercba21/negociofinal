...
(function reportesInit(){
  ...
  // ===== Reportes / Cierre diario (UI mínima) =====
  ...
  const btnRepBuscar = document.getElementById("btnRepBuscar");
  const cierreFecha = document.getElementById("cierreFecha");
  const btnCierreCrear = document.getElementById("btnCierreCrear");
  const repOut = document.getElementById("repOut");

  function todayIso() {
    const d = new Date();
    // input[type=date] usa YYYY-MM-DD local
  ...
  async function cargarReportes() {
    ...
    const qEstado = estado ? `&estado=${encodeURIComponent(estado)}` : "";
    const resumen = await fetchJSON(`/arca/reportes/resumen?desde=${desde}&hasta=${hasta}${qEstado}`);
    const comps = await fetchJSON(`/arca/reportes/comprobantes?desde=${desde}&hasta=${hasta}${qEstado}`);
    ...
    repOut.innerHTML = `
      <div style="display:grid;gap:12px;margin-top:8px;">
        <div style="padding:12px;border:1px solid #eee;border-radius:12px;">
          <b>Totales</b><br/>
          Facturas: ${tot.cant_facturas || 0} — NC: ${tot.cant_nc || 0}<br/>
          Total facturas: $${Number(tot.total_facturas || 0).toFixed(2)}<br/>
          Total NC: $${Number(tot.total_nc || 0).toFixed(2)}<br/>
          Ventas netas: $${Number(tot.ventas_netas || 0).toFixed(2)}
        </div>

        <div style="padding:12px;border:1px solid #eee;border-radius:12px;overflow:auto;">
          <b>Resumen por día</b>
          <table style="width:100%;border-collapse:collapse;margin-top:8px;">
            <thead>
              <tr>
                <th style="text-align:left;border-bottom:1px solid #eee;padding:6px;">Fecha</th>
                <th style="text-align:right;border-bottom:1px solid #eee;padding:6px;">Facturas</th>
                <th style="text-align:right;border-bottom:1px solid #eee;padding:6px;">NC</th>
                <th style="text-align:right;border-bottom:1px solid #eee;padding:6px;">Ventas netas</th>
              </tr>
            </thead>
            <tbody>
              ${dias.map(d => `
                <tr>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;">${d.fecha}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;text-align:right;">${d.cant_facturas}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;text-align:right;">${d.cant_nc}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;text-align:right;">$${Number(d.ventas_netas || 0).toFixed(2)}</td>
                </tr>
              `).join("")}
            </tbody>
          </table>
        ...
        <div style="padding:12px;border:1px solid #eee;border-radius:12px;overflow:auto;">
          <b>Comprobantes</b> (${(comps.comprobantes || []).length})
          <table style="width:100%;border-collapse:collapse;margin-top:8px;">
            <thead>
              <tr>
                <th style="text-align:left;border-bottom:1px solid #eee;padding:6px;">ID</th>
                <th style="text-align:left;border-bottom:1px solid #eee;padding:6px;">Fch</th>
                <th style="text-align:right;border-bottom:1px solid #eee;padding:6px;">Tipo</th>
                <th style="text-align:right;border-bottom:1px solid #eee;padding:6px;">Nro</th>
                <th style="text-align:right;border-bottom:1px solid #eee;padding:6px;">Total</th>
              </tr>
            </thead>
            <tbody>
              ${(comps.comprobantes || []).map(c => `
                <tr>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;">${c.id}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;">${c.cbte_fch}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;text-align:right;">${c.cbte_tipo}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;text-align:right;">${c.cbte_nro}</td>
                  <td style="padding:6px;border-bottom:1px solid #f3f3f3;text-align:right;">$${Number(c.imp_total || 0).toFixed(2)}</td>
                </tr>
              `).join("")}
            </tbody>
          </table>
  ...
  async function crearCierre() {
    ...
    const r = await fetchJSON(`/arca/cierres-diarios`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ fecha })
    });
    alert(`Cierre creado OK. id=${r.id} cbtes=${r.cant_cbtes}`);
  }
  ...
  if (repDesde && repHasta && cierreFecha) {
    ...
    btnRepBuscar?.addEventListener("click", cargarReportes);
    btnCierreCrear?.addEventListener("click", crearCierre);
  }
